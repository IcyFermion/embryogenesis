...
\section*{Applicability to the adopted Figure 8 reference}
As of 9 September 2026, Figure 8 uses the separate-clock model specified in
\texttt{separate\_clock\_reference.tex}. The derivation below remains valid,
but is applied separately to spatial increments divided by $\sqrt{t_e}$
and to unscaled protein increments (one unit per canonical transition).
The protein block is no longer divided by tracking duration because atlas
protein states are not endpoint measurements aligned to the tracking cutoff.
References below to Equation 2 of the companion note concern the archived
\texttt{parametric\_brownian\_bootstrap.tex} specification.
